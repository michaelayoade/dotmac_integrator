...
`dotmac-integration 0.1.0a14` declares the database roles, idempotency ledger and
append-only platform audit log requirements. All three are bound below, and
nothing else is.

Re-derived at the a13 → a14 bump, which is required rather than a formality: a
...
than a shrug — a14 advances the lineage to `ig_0012_delivery_evidence`, but its
`manifest.requires` tuple is byte-identical to a13's. The release adds columns
and a legal-hold table inside the module's existing platform schema; it consumes
no new kernel effect. A no-op binding re-derivation is recorded because the
alternative is a reader at the next bump not knowing whether it was checked.
...
## The `ig_0001` literal edge — unrepaired through a14

`ig_0001_connector_cp` still ships `depends_on = ("0001_initial_tenant_schema",)`
at `0.1.0a14` — a physical edge naming a foreign revision, which is exactly what
