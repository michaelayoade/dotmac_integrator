...
def test_the_upgrade_itself_verified_the_prerequisites(migrated: str) -> None:
    ...
    fact that makes an order canary the wrong instrument. `ig_0014` is the head
    at a16, so that row is the right question to ask; its ancestry includes the
    ...
    engine.dispose()
    assert "ig_0014_polling_evidence" in applied, (
        f"alembic_version holds {sorted(applied)}. The `ig` head at "
        "dotmac-integration 0.1.0a16 is ig_0014_polling_evidence, whose ancestry "
