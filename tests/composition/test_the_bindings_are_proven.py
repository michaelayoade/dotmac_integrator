...
def test_the_upgrade_itself_verified_the_prerequisites(migrated: str) -> None:
    ...
    fact that makes an order canary the wrong instrument. `ig_0012` is the head
    at a14, so that row is the right question to ask; its ancestry includes the
    ...
    engine.dispose()
    assert "ig_0012_delivery_evidence" in applied, (
        f"alembic_version holds {sorted(applied)}. The `ig` head at "
        "dotmac-integration 0.1.0a14 is ig_0012_delivery_evidence, whose ancestry "
